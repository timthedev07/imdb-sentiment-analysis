...
def readDataToDF():
    """
    This returns a tuple, (train, test), with two pandas data frames; one contains training data while the other contains testing data
    """
        ...
        for label in labels.keys():
            ...
            for reviewTxtFname in os.listdir(currLabelDir):
                with open(os.path.join(currLabelDir, reviewTxtFname), "r", encoding='utf-8') as f:
                    raw = f.read().strip("\n")
                    df = df.append([[raw, labels[label]]], ignore_index=True)
